...
switch ($options[xPDOTransport::PACKAGE_ACTION]) {
    ...
    case xPDOTransport::ACTION_UPGRADE:
        $setting = $modx->getObject(modSystemSetting::class, ['key' => 'quip.emailsTo']);
        if ($setting != null) { $values['emailsTo'] = $setting->get('value'); }
        unset($setting);

        $setting = $modx->getObject(modSystemSetting::class, ['key' => 'quip.emailsFrom']);
        if ($setting != null) { $values['emailsFrom'] = $setting->get('value'); }
        unset($setting);

        $setting = $modx->getObject(modSystemSetting::class, ['key' => 'quip.emailsReplyTo']);
