...
class quipCommentNotify extends \xPDO\Om\xPDOSimpleObject
{
     ...
     * @return boolean
     */
    public function send(quipComment $comment, array $properties = []) {
        ...
        $properties['unsubscribeUrl'] = $comment->makeUrl('', ['quip_unsub' => $email, 'quip_uhsh' => $hash], ['scheme' => 'full'], false) . '#quip-success-' . $comment->get('idprefix');
        $properties['unsubscribeText'] = $this->xpdo->lexicon('quip.unsubscribe_text', ['unsubscribeUrl' => $properties['unsubscribeUrl']]);
