...
class Quip {
    ...
    public function initDebug() {
        if ($this->modx->getOption('debug', $this->config, false)) {
            ...
            $user = $this->modx->getObject(modUser::class, ['username' => $debugUser]);
            if ($user == null) {
     ...
     */
    public function buildPagination(array $options = []) {
        ...
        for ($i=0; $i<$pageCount; $i++) {
            ...
            $u = $options['url'] . (strpos($options['url'], '?') !== false ? '&' : '?') . http_build_query(array_merge($params, [
                'quip_start' => $newStart,
                'quip_limit' => $options['limit']
            ]));
            if ($i != $curPage) {
                $pages .= $this->getChunk($tplItem, [
                    'url' => $u,
                    'idx' => $i+1,
                    'cls' => $pageCls
                ]);
            } else {
                $pages .= $this->getChunk($tplCurrentItem, [
                    'idx' => $i+1,
                    'cls' => $pageCls . ' ' . $currentPageCls
                ]);
            }
        }
        return $this->getChunk($this->modx->getOption('tplPagination', $options, 'quipPagination'), [
            'pages' => $pages,
    ...
    public function getTimeAgo($time = '') {
        ...
        if (!empty($agoTS['months'])) {
            $ago[] = $this->modx->lexicon(($agoTS['months'] > 1 ? 'ago_months' : 'ago_month'), ['time' => $agoTS['months']]);
        }
        ...
        if (!empty($agoTS['minutes']) && empty($agoTS['days']) && empty($agoTS['weeks']) && empty($agoTS['months']) && empty($agoTS['years'])) {
            $ago[] = $this->modx->lexicon('ago_minutes', ['time' => $agoTS['minutes']]);
        }
        if (empty($ago)) { /* handle <1 min */
            $ago[] = $this->modx->lexicon('ago_seconds', ['time' => $agoTS['seconds']]);
        }
        $output = implode(', ', $ago);
     ...
     */
    public function loadHooks($type = 'post', array $config = []) {
        $type = $type . 'Hooks';
     ...
     */
    public function cleanse($body, array $scriptProperties = []) {
     ...
     */
    public function parseLinks($body, array $scriptProperties = []) {
    ...
    /**
     * Load the appropriate snippet
     * @param string $className
     * @return null|BaseSnippet
     */
    public function loadSnippet($className) {
        $snippet = null;
        if (class_exists($className)) {
            $snippet = new $className($this, $this->config);
        } else {
            $this->modx->log(modX::LOG_LEVEL_ERROR, '[Quip] Class ' . $className . ' doesn\'t exist.');
        }
        return $snippet;
    }
}
